...
final tokenMangeProvider = Provider<TokenManage>((ref) => TokenManage());

// 토큰 관리 로직
class TokenManage {
  // Single-ton pattern (클래스의 인스턴스가 1개만 생성되도록 하는 디자인 패턴)
  static final TokenManage _instance = TokenManage._internal();
  final FlutterSecureStorage _tokenStorage = const FlutterSecureStorage();

  // Private Constructor
  TokenManage._internal();
  ...
  Future<void> saveToken(TokenModel tokenModel) async {
    String jsonData = jsonEncode(tokenModel.toJson());
